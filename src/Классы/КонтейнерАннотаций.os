...
Функция ДобавитьАннотацию(ТипАннотации, Знач ПереопределитьЕслиСуществует = Ложь) Экспорт
	...
	// Проверка на дублирование аннотации
	СуществующееОпределение = ОпределенияАннотаций.Получить(НРег(ИмяАннотации));
	Если СуществующееОпределение <> Неопределено И НЕ ПереопределитьЕслиСуществует Тогда
		ВызватьИсключение СтрШаблон(
			"Аннотация с именем ""%1"" уже зарегистрирована в контейнере",
			ИмяАннотации
		);
	КонецЕсли;

	ОпределениеАннотации = Новый ОпределениеАннотации(ИмяАннотации, ИмяТипаАннотации, Параметры, Аннотации, ЭтотОбъект);
