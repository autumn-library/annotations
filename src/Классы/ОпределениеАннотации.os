...
// Значения-параметры, являющиеся вложенными аннотациями, рекурсивно материализуются в объекты
// соответствующих аннотаций, если их определения зарегистрированы в контейнере данного определения.
//
// Параметры:
//   Аннотация - СтрокаТаблицыЗначений - Данные аннотации:
//     * Имя       - Строка          - Имя аннотации.
...
//                   - СтрокаТаблицыЗначений - вложенная аннотация (см. РаботаСАннотациями.ЭтоОписаниеАннотации):
//         *** Имя       - Строка          - Имя вложенной аннотации.
//         *** Параметры - ТаблицаЗначений  - Параметры вложенной аннотации.
//
// Возвращаемое значение:
...
//
// Если значение является вложенной аннотацией (см. РаботаСАннотациями.ЭтоОписаниеАннотации),
// а её определение зарегистрировано в контейнере данного определения, вложенная аннотация
// рекурсивно превращается в объект своей аннотации. В остальных случаях (литерал либо
// незарегистрированная аннотация) значение возвращается как есть.
//
// Параметры:
...
Функция МатериализоватьЗначение(Значение)
	...
	Если НЕ РаботаСАннотациями.ЭтоОписаниеАннотации(Значение) Тогда
		Возврат Значение;
	КонецЕсли;
